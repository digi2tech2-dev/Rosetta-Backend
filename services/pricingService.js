...
async function buildServerCart(customerId) {
  ...
  for (const cartItem of cart.items) {
    ...
    items.push({
      ...
      costPrice: product.pCost !== null && product.pCost !== undefined && product.pCost !== "" && Number.isFinite(Number(product.pCost)) && Number(product.pCost) >= 0 ? fromCents(toCents(product.pCost)) : null,
